...
def solve(batches):
    ...
    # This is slow because of curr.

    inp = first
    result = all_outcomes(inp)
    listall = list(result)
    this_batch = 1
    calculate_probabilities(this_batch, listall)
    this_batch += 1
    while this_batch < batches:
        current = []
        for i in listall:
            curr = all_outcomes(i)
            for j in curr:
...
def calculate_probabilities(round, batch):
    ...
    for outcome in batch:
        if sum(outcome) == 1:
            tally += 1
    print "Batch: ", round
    print "One-Paper Occurences: ", tally
    print "Total Equiprobable Outcomes: ", len(batch)
    print "Probability: ", float(tally) / float(len(batch))
    print "======================="
